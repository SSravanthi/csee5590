...
public class EmployeeActivity extends AppCompatActivity {
    ...
    // Update in DB code-
    private void updateinDB(){ // getting connected to database for updating
        SQLiteDatabase database = new SampleDBSQLiteHelper(this).getWritableDatabase();
        ContentValues values = new ContentValues();
        String firstname = binding.firstnameEditText.getText().toString();// values in database example lastname,job description
        values.put(SampleDBContract.Employee.COLUMN_LASTNAME, binding.lastnameEditText.getText().toString());
        values.put(SampleDBContract.Employee.COLUMN_JOB_DESCRIPTION, binding.jobDescEditText.getText().toString());
        values.put(SampleDBContract.Employee.COLUMN_EMPLOYER_ID,
                ((Cursor) binding.employerSpinner.getSelectedItem()).getInt(0));
        ...
        try {
            ...
            Log.e(TAG, "Error", e);
            Toast.makeText(this, "Not Updated", Toast.LENGTH_LONG).show();// will make a text if it has error
            return;
        }
    ...
    private void deleteFromDB(){    // Gets Connected to database for delete function
        SQLiteDatabase database = new SampleDBSQLiteHelper(this).getReadableDatabase();
        String firstname = binding.firstnameEditText.getText().toString();
        database.delete(SampleDBContract.Employee.TABLE_NAME, SampleDBContract.Employee.COLUMN_FIRSTNAME + "='" + firstname + "'",null);
        // Will delete the first name from Employee database
        Toast.makeText(this, "Employee- " + firstname + "  is deleted from database", Toast.LENGTH_LONG).show();
        // Shows the text that First name is deleted from database successfully
    }
